    ...
    if User.objects.filter(username=username).exists():
        ...
        if User.objects.filter(email=email).exists():
            ...
            user = User.objects.create_user(
                first_name=fn, last_name=ln, username=username, password=password, email=email)
            user.save()

            if category == "tutor":
                Tutor(tid=int(Tutor.objects.all().count()) + 1, pic=None, name=fn,
                      star=0, about="null", taught=0, rate=0, address="null").save()
...
def searchresults(request):
    ...
    results = Subject.objects.filter(
        subject__icontains=sub, level__icontains=level, mode__icontains=mode).select_related('tid')
...
def pswdchanged(request):
    ...
    if check_password(old, User.objects.get(username=request.user.username).password):
        ...
        p.save()
        send_mail('Password changed!', 'Hello ' + request.user.first_name + ", Your password has been changed successfully!",
                  'guruhubportal@gmail.com', [request.user.email], fail_silently=False)
...
def mailsent(request):
    ...
    if User.objects.filter(username=username).exists():
        ...
        p.save()
        send_mail('Password changed!', 'Hello ' + username + ", Your new password is " +
                  "'"+str(res)+"'", 'guruhubportal@gmail.com', [email], fail_silently=False)
...
def detailsadded(request):
    ...
    path = os.path.join(
        "D:\SEM - 6\Capstone\GuruHub\\tutionfinder\photos", str(profilepic))
    shutil.copy(path, "D:/SEM - 6/Capstone/GuruHub/tutionfinder/media/pics")
...
def subjectadded(request):
    ...
    Subject.objects.create(id=int(Subject.objects.all().count(
    )) + 1, subject=subject, level=subjectlevel, mode=mode, tid_id=tid_id)
...
def givestar(request):
    ...
    if request.user.is_authenticated:
        ...
        if isinstance(star, int) and star <= 5:
            ...
            b = ((starobj.star*(int(starobj.starcount)-1)) + star) / \
                int(starobj.starcount)
